...
use syn::{
    ...
    Ident, Lifetime, LitStr, Result, Token,
};
...
impl Product {
    fn as_sql(&self, table_name: &String, bindings: &BTreeMap<&Ident, &LitStr>) -> String {
        let bindings = self
            .idents
            .iter()
            .map(|ident| {
    ...
    fn as_query_by_ident(&self) -> Ident {
        let idents = self
            .idents
            .iter()
    ...
    pub fn as_orm_method(&self, table_name: &String, bindings: &Bindings) -> TokenStream {
        ...
        let named_params = self
            ...
            .collect::<Vec<_>>();
        let generic_lifes = (0..named_params.len())
            .map(|i| Lifetime::new(format!("'t{}", i).as_str(), Span::call_site()))
            .collect::<Vec<_>>();
        let generic_types = (0..named_params.len())
            .map(|i| {
                let vartype = format_ident!("T{}", i);
                quote! { #vartype }
            })
            .collect::<Vec<_>>();
        let type_bounds = zip(generic_lifes.clone(), generic_types.clone())
            .map(|(varlife, vartype)| {
        ...
        let arguments = zip(
            self.idents.clone(),
            zip(generic_lifes.clone(), generic_types.clone()),
        )
        .map(|(ident, (varlife, vartype))| {
